...
export default function App() {
  ...
  async function funct_init(){
    ...
    Accelerometer.addListener(data => {
      ...
      const newData = { ...data, timestamp, activity, sensor };
  ...
  async function funct_send(){
    ...
    // Verificamos si datosCombinados no está vacío antes de cargar el archivo JSON en el bucket de S3
       ...
       <Modal visible={showOptions} animationType="slide">
        ...
        <Text style={styles.text}>Registre la actividad que está realizando: </Text>
          <TouchableOpacity style={styles.modalButton} onPress={async () => {
            ...
            await new Promise(resolve => setTimeout(resolve, 0)); // esperar a que se complete la asignación
            await funct_init();
